...
def sum_reduction_kernel[
    ...
    var i = thread_idx.x
    var partial_sum: segment.element_type
    if i < segment_len:
    ...
    for c in range(1, coarse_factor * 2):
        if i + c * block_dim < segment_len:
            partial_sum += segment[i + c * block_dim]
        else:
            break
    ...
    if warp_idx == 0:
        ...
        partial_sum = warp_reduce(partial_sum)
        if thread_idx.x == 0:
            _ = Atomic.fetch_add(output, partial_sum[0])
...
def sum_reduction_gpu[
    ...
    ctx.enqueue_function[
        sum_reduction_kernel[dtype, block_dim, coarse_factor],
        sum_reduction_kernel[dtype, block_dim, coarse_factor],
    ](
        input_tensor,
        d_output,
