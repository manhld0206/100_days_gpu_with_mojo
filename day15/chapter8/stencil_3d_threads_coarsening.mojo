...
def stencil_3d_kernel[
    ...
    var in_s = LayoutTensor[
        dtype,
        Layout.row_major(in_s_z_dim, block_dim, block_dim),
        MutAnyOrigin,
    ...
    for i in range(2 * stencil_radius):
        ...
        if (
            ...
            in_s[i, ty, tx] = input[dep, row, col]

    barrier()

    var current_in_s_z = stencil_radius

    for dep in range(dep_start, dep_start + out_dim):
        # Load last layer only
        if (
            col < n
            and col >= 0
            and row < n
            ...
            in_s[
                (current_in_s_z + stencil_radius) % in_s_z_dim, ty, tx
            ] = input[dep, row, col]
        barrier()
        ...
        if (
            ...
            if (
                ...
                var value = coefficients[0] * in_s[current_in_s_z, ty, tx]
                comptime for i in range(0, stencil_radius):
                    value += (
                        coefficients[1 + i]
                        * in_s[
                            (current_in_s_z - (stencil_radius - i))
                            % in_s_z_dim,
                            ty,
                            tx,
                        ]
                ...
                comptime for i in range(0, stencil_radius):
                    value += (
                        ...
                        * in_s[(current_in_s_z + i) % in_s_z_dim, ty, tx]
                    )
                comptime for i in range(0, stencil_radius):
                    value += (
                        coefficients[1 + 2 * stencil_radius + i]
                        * in_s[current_in_s_z, ty - (stencil_radius - i), tx]
                    )
                comptime for i in range(0, stencil_radius):
                    value += (
                        coefficients[1 + 3 * stencil_radius + i]
                        * in_s[current_in_s_z, ty + i, tx]
                    )
                comptime for i in range(0, stencil_radius):
                    value += (
                        coefficients[1 + 4 * stencil_radius + i]
                        * in_s[current_in_s_z, ty, tx - (stencil_radius - i)]
                    )
                comptime for i in range(0, stencil_radius):
                    value += (
                        coefficients[1 + 5 * stencil_radius + i]
                        * in_s[current_in_s_z, ty, tx + i]
                    )
        ...
        current_in_s_z = (current_in_s_z + 1) % in_s_z_dim
