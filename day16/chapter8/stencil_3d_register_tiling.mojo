...
def stencil_3d_kernel[
    ...
    comptime in_s_z_dim = 2 * stencil_radius + 1
    ...
    var in_s = LayoutTensor[
        dtype,
        Layout.row_major(block_dim, block_dim),
        MutAnyOrigin,
        address_space=AddressSpace.SHARED,
    ].stack_allocation()
    var in_array = InlineArray[input.element_type, in_s_z_dim](fill=0)

    var col = block_idx.x * out_dim + thread_idx.x - stencil_radius
    var row = block_idx.y * out_dim + thread_idx.y - stencil_radius
    var dep_start = block_idx.z * out_dim

    var tx = thread_idx.x
    var ty = thread_idx.y

    # The last layer will be loaded at the start of the loop
    for i in range(in_s_z_dim - 1):
        var dep = dep_start + i - stencil_radius
        if (
            col < n
            and col >= 0
            and row < n
            ...
            in_array[i] = input[dep, row, col]

    if col < n and col >= 0 and row < n and row >= 0:
        in_s[ty, tx] = input[dep_start, row, col]

    barrier()

    var current_index = stencil_radius

    for dep in range(dep_start, dep_start + out_dim):
        # Load last element only
        if (
            col < n
            and col >= 0
            and row < n
            and row >= 0
            and dep + stencil_radius < n
            and dep + stencil_radius >= 0
        ):
            in_array[(current_index + stencil_radius) % in_s_z_dim] = input[
                dep, row, col
            ]
        barrier()
        ...
        if (
            ...
            if (
                ...
                var value = coefficients[0] * in_array[current_index]
                comptime for i in range(0, stencil_radius):
                    value += (
                        coefficients[1 + i]
                        * in_array[(current_index - (stencil_radius - i))]
                    )
                comptime for i in range(0, stencil_radius):
                    value += (
                        coefficients[1 + stencil_radius + i]
                        * in_array[(current_index + i) % in_s_z_dim]
                    )
                comptime for i in range(0, stencil_radius):
                    value += (
                        coefficients[1 + 2 * stencil_radius + i]
                        * in_s[ty - (stencil_radius - i), tx]
                    )
                comptime for i in range(0, stencil_radius):
                    value += (
        ...
        barrier()
        current_index = (current_index + 1) % in_s_z_dim
        in_s[ty, tx] = in_array[current_index]
