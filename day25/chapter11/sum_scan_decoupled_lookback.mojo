...
@fieldwise_init
struct FlagEnum(Equatable, ImplicitlyCopyable):
    var _value: Int32

    comptime NOT_READY = FlagEnum(0)._value
    comptime SUM_READY = FlagEnum(1)._value
    comptime PREFIX_READY = FlagEnum(2)._value
...
def inter_block_scan[
    ...
    running_prefix: Scalar[dtype] = 0
    if thread_idx.x == block_dim - 1:
        if bid > 0:
            lookback = 1
            while True:
                flag = Atomic.fetch_add[ordering=Ordering.ACQUIRE](
                    flags.ptr_at_offset(Index(bid - lookback)), 0
                )
                if flag == FlagEnum.SUM_READY:
                    if bid - lookback > 0:
                        running_prefix += partial_sums[bid - lookback][0]
                        lookback += 1
                elif flag == FlagEnum.PREFIX_READY:
                    previous_block_partial_sum[0] = (
                        prefixes[bid - lookback][0] + running_prefix
                    )
                    break
        else:
            previous_block_partial_sum[0] = 0

        prefixes[bid] = previous_block_partial_sum[0] + val
...
def sum_scan_kernel[
    ...
    comptime assert coarse_factor % simd_width == 0
    comptime num_simd_load = coarse_factor / simd_width
    ...
    var buffer_r = LayoutTensor[
        dtype,
        Layout.row_major(coarse_factor),
        MutAnyOrigin,
        address_space=AddressSpace.LOCAL,
    ].stack_allocation()
    # var buffer_r = SIMD[dtype, coarse_factor](0)

    if segment.dim(0) == coarse_factor:
        comptime for i in range(num_simd_load):
            buffer_r.store[width=simd_width](
                Index(i * simd_width),
                val=segment.load[width=simd_width](Index(i * simd_width)),
            )
        comptime for c in range(1, coarse_factor):
            buffer_r[c] = buffer_r[c - 1] + buffer_r[c]

    elif segment.dim(0) > 0:
    ...
    var thread_sum = buffer_r[coarse_factor - 1][0]
    ...
    if thread_id == block_dim - 1:
        partial_sums[bid] = buffer_r[coarse_factor - 1]
        _ = Atomic.fetch_add[ordering=Ordering.RELEASE](
            flags.ptr_at_offset(Index(bid)), 1
        )

    previous_block_partial_sum = inter_block_scan[dtype, block_dim](
        buffer_r[coarse_factor - 1][0], bid, partial_sums, prefixes, flags
    )
    ...
    if segment.dim(0) == coarse_factor:
        comptime for i in range(num_simd_load):
            output_segment.store[width=simd_width](
                Index(i * simd_width),
                val=buffer_r.load[width=simd_width](Index(i * simd_width)),
            )
...
def sum_scan_gpu[
    ...
    var d_prefixes = ctx.enqueue_create_buffer[dtype](num_blocks)
    ...
    var prefixes_tensor = LayoutTensor[
        mut=True, dtype, Layout.row_major(UNKNOWN_VALUE), MutAnyOrigin
    ](d_prefixes, layout_num_blocks)
